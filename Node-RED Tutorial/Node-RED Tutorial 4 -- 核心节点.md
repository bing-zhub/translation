...
每当接收到输入消息时，创建两个输出消息，间隔可配置的时间间隔。  也可以用作监控定时器。
...
该节点根据其属性路由消息。  属性使用UI进行配置，并且可以是应用于消息属性的各种逻辑（>，<，\> =等）。
...
使用可配置的选择器（CSS选择器语法）从msg.payload中的html文档中提取元素。  本质上允许您解析HTML并返回匹配的元素的数组。
